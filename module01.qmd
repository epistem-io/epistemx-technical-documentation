...
3.  Next, the system validates and repairs the shapefile geometries (if any errors are found) in a GeoDataFrame (`.GDF`) and converts the cleaned geometries into an Earth Engine object.
4.  The system displays a small preview map centered on the AOI.
...
2.  A summary report containing the metadata is provided as a `.TXT` file that includes:
    -   Total images found
