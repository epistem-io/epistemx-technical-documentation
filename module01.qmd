...
2.  The system currently supports Landsat 1-4 at sensor radiance and Landsat 5-9 collection 2 surface reflectance Analysis Ready Data (ARD). Landsat mission availability is as follows:
...
`get_optical_data()` to retrieve and filter the Landsat image collection based on the input parameters.
...
`reflectance_data()` to fetch and analyze Landsat optical/thermal image collections.

`get_optical_data()` to retrieve and filter the image collections based on the input parameters.

`rename_landsat_bands ()` to rename bands per sensor type, so all sensors have common names.

`Reflectance_Data.get_optical_data()` to get reflectance data from all input parameters.

`ee.Filter.lt()` to remove images exceeding cloud cover limit.

`parse_year_or_date()` to convert year input (e.g., 2022) into full date strings.

`ee.ImageCollection()` to load a satellite image collection.

`.filterBounds()` to clip and mosaic the satellite image to AOI.

`.filterDate()` to filter the image collection by a date range.

`mask_landsat_sr()` to mask cloud and cloud shadow.

`self.logger.info()` to logs informational message.

`self.logger.warning()` to logs warning if images not found.

`Reflectance_Stats.get_collection_statistics()` to compute summary statistic of reflectance values of an ee.ImageCollection over an AOI and time range.
