...
## Visualization and Saving Data {#sec-visualization-and-saving-data}
...
::: callout-tip
## Main Related Function(s)

List of functions in `Reflectance_Data` Class:

1.  `\_\_init\_\_` to create a logger and logs initialization.
2.  `mask_landsat_sr()` to mask cloud and cloud shadow.
3.  `rename_landsat_bands()` to rename bands per sensor type, so all sensors have common names.
4.  `get_optical_data()` to retrieve and filter the image collections based on the input parameters.

Nested Helper Functions inside `get_optical_data()`:

1.  `parse_year_or_date()` to convert year input (e.g., 2022) into full date strings.
2.  `add_aoi_cloud()` to calculate the percentage of cloud coverage within AOI for each image.

List of functions in `Reflectance_Stats` Class:

1.  `\_\_init\_\_` to create a logger and logs initialization.
2.  `get_collection_statistics()` to claculate the image collections' statistics.
3.  `print_collection_report()` to display the image collection statistics report.
:::
