...
3.  The system runs the separability analysis. Both methods TM and TD take the spectral pixel data of two classes and calculate how distinct they are based on their means and covariance matrices. The resulting values range from 0 (no separability) to 2 (perfect separability).
...
4.  Finally, the result matrix is converted into a readable, cleaned, long-format pandas DataFrame.
...
5.  The system provides a function to filter and highlight the class pairs that are most difficult to distinguish, based on their separability values.
...
6.  The separability values are assigned to a categorical level indicating how well two classes can be separated. The system groups the numeric values into these categories.
...
7.  The system summarizes the number of class pairs in each category to provide an overall view of class separability.
...
8.  Results, along with the basic statistics table, are exported as a `.TXT` report.
9.  Detailed error messages are provided if any errors occur.
