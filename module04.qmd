...
`get_separability_df()` to compute class separability. `lowest_separability()` to retrieve the class pair with the lowest separability value. `separability_level()` to categorize the separability values into Good, Waek, or Confusion. `sum_separability ()` to summarize the separability results.
:::
...
`plot_facet_histograms()` to plot faceted histograms of the bands.

`plot_boxplots_by_band()` to plot boxplots of the bands.

`print_analysis_summary()` to summarize the training dataset, showing class balance, class separability, and the most problematic class pairs.
:::
