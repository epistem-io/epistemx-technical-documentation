# Module 6: LULC Map Generation {#sec-module-6}
...
`Hard_classification_tuning()` to perform manual testing to find optimal parameters for multiclass hard classification using training data from single or stratified random split.

`Soft_classification_tuning()` to perform manual testing to find optimal parameters for one-vs-rest binary soft classification using training data from single or stratified random split.

`hard_tuning_kfold()` to perform manual testing to find optimal parameters for multiclass hard classification with stratified k-fold input data.

`soft_tuning_kfold()` to perform manual testing to find optimal parameters for one-vs-rest binary soft classification with k-fold cross-validation data.
:::
...
`random_split()` to perform single random split and extract pixel values from the imagery.

`stratified_split()` to perform stratified random split to partition the sample data into training and testing sets.

`stratified_kfold()` to perform stratified k-fold cross-validation split on input reference data.

`inspect_fold()` to inspect a specific fold's sizes from stratified k-fold results.
:::
...
`multiclass_classification()` to perform multiclass hard classification to generate land cover land use map using Random Forest with specified parameters.

`ovr_classification()` to implement one-vs-rest binary classification for multi-class land cover, generating probability stacks and optional final map via argmax.
:::
