...
`sample_stats ()` to compute training data statistics: counts the number of samples per class, total number of samples, gets unique class, calculates class balance (proportion of each class).
:::
...
`get_sample_stats_df ()` to convert training data table to pandas DataFrame.
:::
