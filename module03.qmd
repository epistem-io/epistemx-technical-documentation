...
<!-- 1.  User-selected AOI (@sec-selection-of-aoi). The image submitted to this module is already in the AOI  -->
...
`training_data` : ee.FeatureCollection - Training polygons/points `image`: ee.Image - The image to extract spectral values `class_property` : str - Property/columen containing class ID (unique) `region` : ee.Geometry - Optional region to limit analysis `class_name_property`: str - Property/column containing class name
:::
...
2.  If the user does not have their own training data, the system will continue to @user-dont-have-training-data.
...
1.  The system provides an upload interface that accepts only `.SHP` and `.ZIP` file (containing point features with LULC class attributes) or `.CSV`/`.XLS`/`.XLSX` format (containing point coordinates and associated LULC class labels established in [Module 2](@sec-module-2)). 
...
### User Does Not Have Training Data {#user-dont-have-training-data}

**Front-end**

The user is given the following options to proceed:

<!-- 1.  Use on-screen sampling. Feature not developed yet-->
...
## Verifying the Uploaded Training Data {#sec-verifying-the-uploaded-training-data}

**Front-end**

1.  The user receives a notification indicating the status of their uploaded sampling data:

    -   Verification success - the user can proceed to @sec-preview-of-verified-training-data
    -   Verification fail - the number of sample points for one or more classes is inadequate. 
    <!-- If verification failed, the user can either: -->
...
6.  Next, the system evaluates the adequacy of sample data quantity for each class.

::: callout-tip
## Backend function

Not developed yet. Needs error handling of each verification steps
...
The user reviews the training data summary displayed in a table.
...
<!-- ## Data Splitting {#sec-data-splitting} MOVED TO MODULE 6-->

<!-- **Front-end** -->

<!-- The user selects one of the following options for splitting the training data summary: -->
...
<!-- 1.  Use all sample points for training - the system consolidates all data into a single DataFrame or variable, stored as a new object to be used in [Module 4](@sec-module4) and [6](@sec-module-6). -->
